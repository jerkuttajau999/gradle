...
class BuildCacheSymlinksIntegrationSpec extends AbstractIntegrationSpec {
    ...
    def "copying files again should have up-to-date=#upToDate if preserveLinks=#preserveLinks"() {
        ...
        "LinksStrategy.ALL"      | false // should be true
        "LinksStrategy.RELATIVE" | false // should be true
    ...
    def "when link is replaced by its contents, the task should have up-to-date=#upToDate if preserveLinks=#preserveLinks"() {
        checkUpToDate(preserveLinks, upToDate) {
            dirLink.delete()
